...
inline std::ostream& operator<< (std::ostream& stream, const Vec3f& v)
{
	std::streamsize w = stream.width();
	std::ios::fmtflags origFlags = stream.flags();
	stream.setf (std::ios::fixed | std::ios::internal | std::ios::showpos);
	stream.precision(3);
	stream.fill('0');
