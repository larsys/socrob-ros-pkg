							...
							std::vector<T > &inliers)
{
	...
	/*
	std::ofstream dataTXT("dataTXT.txt");
	for (j=0; j<data.size(); j++) {
		dataTXT << data[j].x << " " << data[j].y << "\n";
	}
	dataTXT.close();
	*/
	
	//std::ofstream dataInliers("dataInliers.txt");
	//compute the least squares estimate using the largest sub set
	if(numVotesForBest > 0) {
		parameters.clear();
		for(j=0; j<numDataObjects; j++) {
			if(bestVotes[j]){
				//dataInliers << data[j].x << " " << data[j].y << "\n"; 
	...
	//std::cout << "Numero de in: " << leastSquaresEstimateData.size() << "\n"; 	
