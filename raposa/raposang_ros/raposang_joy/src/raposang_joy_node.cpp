...
bool had_movement, had_arm_change, asked_light, was_ok;
...
void RAPOSA_Joy_Control(const sensor_msgs::Joy &joymsg) {
	...
	if (RAPOSA_Check_Button(joymsg.buttons, pad.button_default_arm)) {
  	...
  	ROS_INFO("[RAPOSANG-JOY] Reset. Inclination sent to RAPOSA (%d).", raposaarm.angle);
		pubA.publish(raposaarm);
  } else if (RAPOSA_Check_Button(joymsg.buttons, pad.button_max_arm)) {
		raposaarm.angle = dir = max_arm;
  	ROS_INFO("[RAPOSANG-JOY] Increment. Inclination sent to RAPOSA (%d).", raposaarm.angle);
		pubA.publish(raposaarm);
	} else if (RAPOSA_Check_Button(joymsg.buttons, pad.button_min_arm)) {		
		raposaarm.angle = dir = min_arm;
  	ROS_INFO("[RAPOSANG-JOY] Decrement. Inclination sent to RAPOSA (%d).", raposaarm.angle);
		pubA.publish(raposaarm);
	} else if (RAPOSA_Check_Axis(joymsg.axes, 2*pad.axis_arm+1)) {
		
		dir = minmax(dir + joymsg.axes[2*pad.axis_arm+1] * pad.default_vel_arm * Turbo, min_arm, max_arm);
	  if (raposaarm.angle != (int) dir) {			
			ROS_INFO("[RAPOSANG-JOY] Inclination sent to RAPOSA (%d, %f).", raposaarm.angle, dir);
			raposaarm.angle = dir;		
			pubA.publish(raposaarm);
		}		
		had_arm_change = true;
				
	} else if (had_arm_change || RAPOSA_Check_Button(joymsg.buttons, pad.button_stop_arm)) {
...
int main(int argc, char *argv[]) {
  ...
  ros::NodeHandle n;
	...
	had_movement = had_arm_change = asked_light = was_ok = false;
