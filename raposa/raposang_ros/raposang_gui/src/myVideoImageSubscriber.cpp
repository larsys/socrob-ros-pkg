...
int globalVr = 0;//virtual pan-tilt 1-yes 0(default)-no default defined in the .launch file
float globalFps=0;
float globalprevFps=0;
...
void callbackHMD (const sensor_msgs::ImageConstPtr& msg, int index) {
  ...
  try {
    pToImageManagmentStruct->pSrcIplImage = bridge.imgMsgToCv ( msg, "rgb8" );
  }
 ...
 *         which represents the ivoid timerCallback(const ros::TimerEvent& foo){
  //if (globalFps!=globalprevFps)
    ROS_INFO("VPN_FPS := %f\n",(1/(globalFps-globalprevFps)));
  globalprevFps=globalFps;
  return;
  }mage that should be shown.
 * Output: None.
 * ---------------------------------------------------------------------
 * Notes:  NONE.
 **********************************************************************/
...
void transitionFromGridToImage ( const sensor_msgs::ImageConstPtr& msg )
{
    ...
    try 
    {
	    globalArrayForSingleImages[indexOfClickedImage]->pSrcIplImage = bridge.imgMsgToCv ( msg, "rgb8" );
    }
	catch (sensor_msgs::CvBridgeException& e)
    {
	   ROS_ERROR ("Could not convert from '%s' to 'rgb8'.", msg->encoding.c_str());
...
void timerCallback(const ros::TimerEvent& foo){
  //if (globalFps!=globalprevFps)
   // ROS_INFO("GUI_FPS := %f\n",((globalFps-globalprevFps)/2));
  globalprevFps=globalFps;
  return;
  }
...
int main(int argc, char **argv)
{
  ...
  image_transport::Subscriber vpn_left;
  image_transport::Subscriber vpn_rigth;
    ...
    // JR - Getting the resizing parameter from the launch file.
    if ( !nh.getParam("virtual_pantilt", globalVr) )    {
        ROS_ERROR ("The globalVr flag was not loaded succesfully. Aborting...\n");
        ros::shutdown();
    }
       // debug purpose only 
       ROS_INFO("globalVr com o valor %d",globalVr);
   ...
   // if (globalVr==0){
       subLeft = nh.subscribe<sensor_msgs::Image> ("leftCameraTopic", 2, boost::bind(imageCallbackWrapper, _1, globalArrayOfImagesForGrid[0], globalArrayForSingleImages[0])); 	
       subRight = nh.subscribe<sensor_msgs::Image> ("rightCameraTopic", 2, boost::bind(imageCallbackWrapper, _1, globalArrayOfImagesForGrid[1], globalArrayForSingleImages[1])); 
       subKinect = nh.subscribe<sensor_msgs::Image> ("kinectCameraTopic", 2, boost::bind(imageCallbackWrapper, _1, globalArrayOfImagesForGrid[2], globalArrayForSingleImages[2]));
       subKinectDepth = nh.subscribe<sensor_msgs::Image> ("kinectDepthCameraTopic", 2, boost::bind(imageCallbackWrapper, _1, globalArrayOfImagesForGrid[3], globalArrayForSingleImages[3]));
   //}
  /*    else{
	vpn_left = it.subscribe ("leftVrPT", 2, boost::bind(imageCallbackWrapper, _1, globalArrayOfImagesForGrid[0], globalArrayForSingleImages[0])); 	
	vpn_rigth = it.subscribe ("rightVrPT", 2, boost::bind(imageCallbackWrapper, _1, globalArrayOfImagesForGrid[1], globalArrayForSingleImages[1])); 
	}; */
