...
class RobustMatcher {
  ...
  public:
      ...
      RobustMatcher() : ratio(0.65f), refineF(true), distance(3.0),confidence(0.99), verbose(false) {
      ...
      cv::Mat match(cv::Mat& image1, cv::Mat& image2, // input images
        ...
        if(verbose)
        {
            std::cout << "Number of points (1): " << keypoints1.size() << std::endl;
            std::cout << "Number of points (2): " << keypoints2.size() << std::endl;
        }
      ...
      // Match feature points using symmetry test and RANSAC
      // returns fundemental matrix
      cv::Mat match(std::vector<cv::DMatch>& matches, // output matches and keypoints
                    std::vector<cv::KeyPoint>& keypoints1, std::vector<cv::KeyPoint>& keypoints2,
