...
Eigen::Matrix4d m4dfromPose(Pose pose)
{
    ...
    pose_ << pose(0,0),pose(0,1),pose(0,2),pose(0,3),
...
//    Note that Euler angles transform objects from the global into the local frame of the vehicle: First yaw rotates around Z (changing X and Y axes to X' and Y'), then pitch around the new Y' axis, and finally roll around the new X' axis.
    Pose Tout;
    Tout.setIdentity();
    Tout =  Eigen::Translation<float,3>(xoffset,yoffset,zoffset)*
            Eigen::AngleAxis<float>(yaw,Eigen::Vector3f::UnitZ()) *
            Eigen::AngleAxis<float>(pitch,Eigen::Vector3f::UnitY()) *
            Eigen::AngleAxis<float>(roll,Eigen::Vector3f::UnitX());
    return Tout;
}
