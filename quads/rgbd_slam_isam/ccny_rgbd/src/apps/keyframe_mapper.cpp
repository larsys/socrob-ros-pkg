  ...
  const CameraInfoMsg::ConstPtr& info_msg)
{
  ...
  int val = cv::countNonZero(frame.depth_img);
  if(val==0)
      return;
  ...
  aruco::CameraParameters param(intr,dist,CamSize);
...
//    std::vector<std::vector<aruco::Marker> > markers = graph_detector_.getMarkers();
...
//    solver_.giveMarkers(markers);
    ...
    filepath = request.filename + "_solved";
    boost::filesystem::create_directories(filepath);
    filepath_keyframes = filepath + "/keyframes/";
    boost::filesystem::create_directories(filepath_keyframes);
    ROS_INFO("Saving keyframes...");
    result_kf = saveKeyframes(keyframes_, filepath_keyframes);
    if (result_kf) ROS_INFO("Keyframes saved to %s", filepath_keyframes.c_str());
    else ROS_ERROR("Keyframe saving failed!");

    ROS_INFO("Saving path...");
    result_path = savePath(filepath);
    savePathTUMFormat(filepath);
    if (result_path ) ROS_INFO("Path saved to %s", filepath.c_str());
    else ROS_ERROR("Path saving failed!");
    solver_.save_graph(filepath);
...
void KeyframeMapper::buildFullCloud(PointCloudT& map_cloud)
{
  PointCloudT::Ptr aggregate_cloud(new PointCloudT());
  aggregate_cloud->header.frame_id = fixed_frame_;
...
//    }
  PointCloudT cloud;
  buildFullCloud(cloud);
  octomap::Pointcloud octomap_cloud;
  for (unsigned int pt_idx = 0; pt_idx < cloud.points.size(); ++pt_idx)
  {
    const PointT& p = cloud.points[pt_idx];
    if (!std::isnan(p.z) && p.z < max_map_z_)
      octomap_cloud.push_back(p.x, p.y, p.z);
  }
  tree.insertScan(octomap_cloud,sensor_origin);
  }
}
