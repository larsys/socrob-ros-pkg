...
#include "ccny_rgbd/apps/rgb_navigator.h"
#include "aruco/aruco.h"
namespace ccny_rgbd {

RGBNavigator::RGBNavigator(
    ...
    rgbd_frame_index_(0)
{
    ROS_INFO("Starting RGBNavigator node");
    ...
    pub_keyframe_service_ = nh_.advertiseService(
                "publish_keyframe", &RGBNavigator::publishKeyframeSrvCallback, this);
    pub_keyframes_service_ = nh_.advertiseService(
                "publish_keyframes", &RGBNavigator::publishKeyframesSrvCallback, this);
    save_kf_service_ = nh_.advertiseService(
                "save_keyframes", &RGBNavigator::saveKeyframesSrvCallback, this);
    load_kf_service_ = nh_.advertiseService(
                "load_keyframes", &RGBNavigator::loadKeyframesSrvCallback, this);
    save_pcd_map_service_ = nh_.advertiseService(
                "save_pcd_map", &RGBNavigator::savePcdMapSrvCallback, this);
    save_octomap_service_ = nh_.advertiseService(
                "save_octomap", &RGBNavigator::saveOctomapSrvCallback, this);
    add_manual_keyframe_service_ = nh_.advertiseService(
                "add_manual_keyframe", &RGBNavigator::addManualKeyframeSrvCallback, this);
    generate_graph_service_ = nh_.advertiseService(
                "generate_graph", &RGBNavigator::generateGraphSrvCallback, this);
    solve_graph_service_ = nh_.advertiseService(
                "solve_graph", &RGBNavigator::solveGraphSrvCallback, this);
    ...
    // Synchronize inputs.
    sync_.reset(new RGBSynchronizer(
                    RGBSyncPolicy(queue_size_), sub_rgb_, sub_info_));

    sync_->registerCallback(boost::bind(&RGBNavigator::RGBCallback, this, _1, _2));
}
...
void RGBNavigator::filter_aruco()
{
    double max_dist = 0.2+0.2+0.2;
    ...
    for(u_int i = 0; i < aruco_pos.size(); i++)
    {
        ...
        {
            double man_dist = std::abs(aruco_pos[i][0]-aruco_pos[j][0])+ std::abs(aruco_pos[i][1]-aruco_pos[j][1])+ std::abs(aruco_pos[i][2]-aruco_pos[j][2]);
    ...
    for(u_int i = 0; i < positions.size(); i++)
    {
        ...
        {
            if(inds[j].size() < 4) //we need min 3 readings to consider a position safe.
            {
                continue;
            }
            double man_dist = std::abs(positions[i][0]-positions[j][0])+ std::abs(positions[i][1]-positions[j][1])+ std::abs(positions[i][2]-positions[j][2]);
            if (man_dist < max_dist)
            {
                sub_inds.push_back(j);
                val[0] += positions[j][0];
        ...
        const CameraInfoMsg::ConstPtr& info_msg)
{
    ...
    out_msg.header   = rgb_msg->header; // Same timestamp and tf frame as input image
    out_msg.encoding = sensor_msgs::image_encodings::TYPE_16UC1; // depth enc
    out_msg.image    = cv::Mat::zeros(rgb_msg->height,rgb_msg->width, CV_8UC1); // Your cv::Mat
    ImageMsg::Ptr depth_msg =  out_msg.toImageMsg();
    ...
    //    }
    addKeyframe(frame, keyframes_[keyframes_.size()-1].pose);
    ...
    graph_detector_.buildCorrespondenceMatrix_rgb_Iterative(keyframes_.size()-1,keyframes_,associations_);

    solver_.add_edges(keyframes_.size()-1,keyframes_,associations_);
    solver_.solve_rgb_Iterative(keyframes_);
...
bool RGBNavigator::processFrame(
...
bool RGBNavigator::publishKeyframeSrvCallback(
...
bool RGBNavigator::publishKeyframesSrvCallback(
...
void RGBNavigator::publishKeyframeData(int i)
{
...
void RGBNavigator::publishKeyframeAssociations()
{
...
void RGBNavigator::publishKeyframePoses()
{
    for(unsigned int kf_idx = 0; kf_idx < keyframes_.size(); ++kf_idx)
    {
        publishKeyframePose(kf_idx);
...
void RGBNavigator::publishArucoPos()
{
    for(u_int i = 0; i < aruco_pos.size(); i++)
    {
        visualization_msgs::Marker marker;
...
void RGBNavigator::publishKeyframePose(int i)
{
    rgbdtools::RGBDKeyframe& keyframe = keyframes_[i];
    if(!keyframe.used && (u_int)i >= solver_.n_original_poses )
        return;
    // **** publish camera pose
...
bool RGBNavigator::saveKeyframesSrvCallback(
...
bool RGBNavigator::loadKeyframesSrvCallback(
...
bool RGBNavigator::savePcdMapSrvCallback(
        Save::Request& request,
        Save::Response& response)
{
...
bool RGBNavigator::saveOctomapSrvCallback(
        Save::Request& request,
        Save::Response& response)
{
...
//void RGBNavigator::createmap(octomap::ColorOcTree* m_octree)
//{
...
bool RGBNavigator::addManualKeyframeSrvCallback(
...
bool RGBNavigator::generateGraphSrvCallback(
...
bool RGBNavigator::solveGraphSrvCallback(
 ...
 */
void RGBNavigator::updatePathFromKeyframePoses()
{
...
bool RGBNavigator::savePcdMap(const std::string& path)
{
    PointCloudT pcd_map;
    buildPcdMap(pcd_map);
...
void RGBNavigator::buildPcdMap(PointCloudT& map_cloud)
{
...
void RGBNavigator::buildFullCloud(PointCloudT& map_cloud)
{
    PointCloudT::Ptr aggregate_cloud(new PointCloudT());
    aggregate_cloud->header.frame_id = fixed_frame_;
...
bool RGBNavigator::saveOctomap(const std::string& path)
{
    bool result;
...
void RGBNavigator::buildColorOctomap(octomap::ColorOcTree& tree)
{
    ROS_INFO("Building Octomap with color...");

    octomap::point3d sensor_origin(0.0, 0.0, 0.0);
    u_int n_original_poses = solver_.getNOriginalPoses();
    for (unsigned int kf_idx = 0; kf_idx < n_original_poses; ++kf_idx)
    {
...
bool RGBNavigator::savePath(const std::string& filepath)
{
    // open file
...
bool RGBNavigator::savePathTUMFormat(const std::string& filepath)
{
    // open file
...
bool RGBNavigator::loadPath(const std::string& filepath)
{
