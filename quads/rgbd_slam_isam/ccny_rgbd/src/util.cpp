...
{
    int max = -1000;
    for (u_int i = 0;i<path_msg.poses.size();i++)
    {
        tf::Pose pose_tf;
        tf::poseMsgToTF(path_msg.poses[i].pose, pose_tf);

        if(pose_tf.getOrigin().z() > max)
            max = pose_tf.getOrigin().z();
    }
    return max;
}
int getminheight(PathMsg path_msg)
{
    ...
    for (u_int i = 0;i<path_msg.poses.size();i++)
    {
        ...
        if(pose_tf.getOrigin().z() < min)
            min = pose_tf.getOrigin().z();
    }
    return min;
}
  ...
  rgbdtools::RGBDFrame& frame)
{
  ...
  frame = rgbdtools::RGBDFrame(rgb_img, depth_img, intr,dist, header);
}
  ...
  PathMsg& path_msg)
{
  ...
  for (u_int idx = 0; idx < path.size(); ++idx)
  {
