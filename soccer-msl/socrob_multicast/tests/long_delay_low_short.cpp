...
{
  init (argc, argv, "long_delay_low_short");
  ...
  TestLongDelay test_long (sid, "LowShort");
  TestShortMultipleDelay short_low_loader (sid, "DUMMY");
  
  {
    Manager srmm (ManagerOptions (sid, 5, "239.255.1.1")
                  ...
                  .short_handler (boost::bind (&TestShortMultipleDelay::short_handler, boost::ref (short_low_loader), _1, _2, _3)));
                  
    Rate loop_rate (10);
    while (ok()) {
      short_low_loader.timer (srmm);
      loop_rate.sleep();
    }
  }
